...
SWEEP_CONFIG = {
    ...
    'parameters': {
        ...
        'hidden_channels': {
            'values': Config.HIDDEN_CHANNELS
        },
        'num_layers': {
            'values': Config.LAYERS
        },
        'focal_alpha': {
            'values': Config.FOCAL_ALPHAS
        },
        'focal_gamma': {
            'values': Config.FOCAL_GAMMAS
        },
        'heads': {
            'values': Config.HEADS
        },
        'concat': {
            'values': Config.CONCAT
        }
    }
}
...
class Experiment:
    ...
    def train_with_wandb(self):
        """Single training run controlled by wandb sweep"""
        ...
        # Create dynamic run name with hyperparameter values
        run_name = f"GAT_lr{config.learning_rate}_h{config.hidden_channels}_l{config.num_layers}_a{config.focal_alpha}_g{config.focal_gamma}_heads{config.heads}_concat{config.concat}"
        
        # Update the run name
        wandb.run.name = run_name
        
        print(f"[Checkpoint] Training run: {run_name}")
        print(f"[Checkpoint] Training with wandb config: {dict(config)}")
        ...
        try:
            ...
            trainer = GATTrainer(model, str(Config.DEVICE), alpha=config.focal_alpha, gamma=config.focal_gamma)
            ...
            if 'best_model_state' in locals():
                ...
                test_trainer = GATTrainer(model, str(Config.DEVICE), alpha=config.focal_alpha, gamma=config.focal_gamma)
                test_metrics = test_trainer.evaluate(self.data_test, calculate_emp=True)
                ...
                wandb.log({
                    ...
                    "test_mp": test_metrics['mp'],
                    "test_lift_0005": test_metrics['lift_0005'],
                    "test_lift_001": test_metrics['lift_001'],
                    "test_lift_005": test_metrics['lift_005'],
                    "test_lift_01": test_metrics['lift_01']
                })
                
                print(f"[Checkpoint] Final test metrics:")
    ...
    def run_wandb_sweep(self, count=20):
        """Initialize and run a wandb sweep"""
        print("[Checkpoint] ====== Starting Wandb Sweep ======")
        
        # Initialize the sweep
        sweep_id = wandb.sweep(SWEEP_CONFIG, project="gat-churn-prediction")
        print(f"[Checkpoint] Created sweep with ID: {sweep_id}")
        print(f"[Checkpoint] View your sweep at: {sweep_id}")
        
        # Run the sweep
        wandb.agent(sweep_id, function=self.train_with_wandb, count=count)
        print(f"[Checkpoint] Completed {count} runs in sweep")

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='GAT Training Script')
    parser.add_argument('--mode', choices=['manual', 'wandb'], default='manual',
                       help='Training mode: manual hyperparameter tuning or wandb sweep')
    parser.add_argument('--sweep-count', type=int, default=20,
                       help='Number of runs in wandb sweep (default: 20)')
    ...
    try:
        ...
        if args.mode == 'wandb':
            print("[Checkpoint] Running wandb sweep mode")
            experiment.run_wandb_sweep(count=args.sweep_count)
        else:
            print("[Checkpoint] Running manual hyperparameter tuning mode")
